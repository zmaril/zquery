...
impl CommandTable {
    fn run_command_remotely(hostname: &str, command: &[String], jc_parser: &str) -> std::io::Result<String> {
    ...
    fn run_command_locally(command: &[String], jc_parser: &str) -> std::io::Result<String> {
        // Run the initial command
        let mut cmd = Command::new(&command[0]);
        if command.len() > 1 {
            cmd.args(&command[1..]);
...
impl TableProvider for CommandTable {
    ...
    async fn scan(
        ...
        let mut output = if self.hostname == "localhost" {
            Self::run_command_locally(&self.command, &self.jc_parser)
                .map_err(|e| DataFusionError::Execution(format!("Failed to execute command: {}", e)))?
        } else {
            Self::run_command_remotely(&self.hostname, &self.command, &self.jc_parser)
                .map_err(|e| DataFusionError::Execution(format!("Failed to execute command: {}", e)))?
        };
...
impl TableFunctionImpl for CommandTableFunc {
    fn call(&self, exprs: &[Expr]) -> Result<Arc<dyn TableProvider>> {
        let hostname = if !exprs.is_empty() && exprs[0].to_string().starts_with("host") {
            //get the argument 
            exprs[0].to_string().split("(").nth(1).unwrap().split(")").next().unwrap().to_string() //lmao terrible, this needs to be fixed later on
        } else {
            "localhost".to_string()
        };
